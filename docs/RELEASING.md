...
- `Scripts/sign-and-notarize.sh`, `Scripts/make_appcast.sh`, and `Scripts/verify_appcast.sh` auto-load `.env` if present.
...
- Zips to `TeamTokenBar-<version>.zip`
- Builds/signs/notarizes/staples `TeamTokenBar-<version>.dmg`
...
- [ ] Upload DMG alongside zip and dSYM on the GitHub release for manual team installs
...
- [ ] Verify on GitHub Releases: assets present (zip, dmg, dSYM, appcast), release notes match changelog, version/tag correct
