...
struct KeychainCopilotTokenStore: CopilotTokenStoring {
    ...
    func loadToken() throws -> String? {
           ...
           now.timeIntervalSince(timestamp) < Self.cacheTTL
        {
            ...
            Self.log.debug("Using cached Copilot token")
            return cached
        }
        if let deniedUntil = Self.deniedUntil {
            if deniedUntil > now {
                Self.cacheLock.unlock()
                Self.log.debug("Skipping Copilot keychain read during prompt cooldown")
                return nil
            }
        ...
        if Self.isPromptDeniedStatus(status) {
            ...
            Self.cache(token: nil, now: now)
            Self.log.info("Copilot keychain access denied; suppressing prompts during cooldown")
            return nil
        }
