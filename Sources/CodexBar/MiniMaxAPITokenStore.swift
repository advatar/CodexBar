...
struct KeychainMiniMaxAPITokenStore: MiniMaxAPITokenStoring {
    ...
    func loadToken() throws -> String? {
           ...
           now.timeIntervalSince(timestamp) < Self.cacheTTL
        {
            ...
            Self.log.debug("Using cached MiniMax API token")
            return cached
        }
        if let deniedUntil = Self.deniedUntil {
            if deniedUntil > now {
                Self.cacheLock.unlock()
                Self.log.debug("Skipping MiniMax keychain read during prompt cooldown")
                return nil
            }
        ...
        if Self.isPromptDeniedStatus(status) {
            ...
            Self.cache(token: nil, now: now)
            Self.log.info("MiniMax keychain access denied; suppressing prompts during cooldown")
            return nil
        }
